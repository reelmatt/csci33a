...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!localStorage.getItem('user')) {
    ...
    // Check if the display name is unique and stop form from submitting
  ...
  socket.on('connect', () => {
    ...
    // Application will emit 'list channel' if channel can be created
    // 'channel error' is emitted if a duplicate
  ...
  socket.on('list channel', channel => {
    // Clear any previous errors
    safeRemove('channelError');
    addNewContent('channels', channel_template({
      'channel': channel
    }));

    // Hide the notification telling users to create a channel to begin
...
/*********************************************************************
 * Add onclick() listeners to channel buttons and message deletions
 *********************************************************************/
document.addEventListener('click', e => {
  // If there is no user, all buttons should not work
  if (!localStorage.getItem('user')) {
    return;
  }
...
function showErrorMessage(elementId, message) {
  ...
  document.getElementById(elementId).innerHTML = error_template({
    'message': message
  });
};
